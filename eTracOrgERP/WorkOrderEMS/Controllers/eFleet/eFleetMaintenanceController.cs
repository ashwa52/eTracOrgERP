...
namespace WorkOrderEMS.Controllers
{
    public class eFleetMaintenanceController : Controller
    {
        ...
        private string MaintenanceDocPath = ConfigurationManager.AppSettings["MaintenanceDoc"];
        ...
        public ActionResult MaintenanceDownload(string Id)
        {
            try
            {
                if (!string.IsNullOrEmpty(Id))
                {
                    Id = Cryptography.GetDecryptedData(Id, true);
                    var _eFleetMaintenanceModel = _IEfleetMaintenance.GeteFleetMaintenanceDetailsById(Convert.ToInt64(Id));
                    if (!string.IsNullOrEmpty(_eFleetMaintenanceModel.MaintenanceAttachment))
                    {
                        string RootDirectory = System.Web.Hosting.HostingEnvironment.ApplicationPhysicalPath;
                        string IsFileExist = RootDirectory + MaintenanceDocPath;
                        RootDirectory = RootDirectory + MaintenanceDocPath + _eFleetMaintenanceModel.MaintenanceAttachment;
                        ...
                        if (Directory.GetFiles(IsFileExist, _eFleetMaintenanceModel.MaintenanceAttachment).Length > 0)
                        {
                            byte[] fileBytes = System.IO.File.ReadAllBytes(RootDirectory);
                            return File(fileBytes, System.Net.Mime.MediaTypeNames.Application.Octet, _eFleetMaintenanceModel.MaintenanceAttachment);
                        }
                        else
                        {
                            RootDirectory = System.Web.Hosting.HostingEnvironment.ApplicationPhysicalPath + MaintenanceDocPath + "FileNotFound.png";
                            byte[] fileBytes = System.IO.File.ReadAllBytes(RootDirectory);
