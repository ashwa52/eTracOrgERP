...
namespace WorkOrderEMS.Controllers.Report
{
    ...
    , string highChart1 = null, string highChart2 = null, bool isTable = false, string tableIdentifier = null, bool isDetailOnly = false)
        ...
        public static void ExporteFleetPDFDetail<T>(IList<T> customerList, string[] columns, string[] columnNamesOnPDF, string filePath, string fileName, bool isMultiple, bool IsDefect,string formDate = null, string toDate = null)
        {
            string value;
            float[] widths;
            string vehiclePath = ConfigurationManager.AppSettings["QRCVehiclePath"];
            string RootDirectory = System.Web.Hosting.HostingEnvironment.ApplicationPhysicalPath;
            vehiclePath = vehiclePath.Replace("~", "");
            string IsFileExist = RootDirectory + vehiclePath;
            try
            {
                iTextSharp.text.Font headerFont = FontFactory.GetFont("Verdana", 11, BaseColor.WHITE);
                iTextSharp.text.Font rowfont = FontFactory.GetFont("Verdana", 10, BaseColor.BLACK);
                ...
                if (true)
                {
                    ...
                    {
                        case "Damage Report":
                            widths = new float[] { 60f, 60f, 70f, 70f, 70f };
                            table.SetWidths(widths);
                            break;
                        case "Defect Report":
                            widths = new float[] { 60f, 60f, 70f, 70f, 70f,70f,70f };
                            table.SetWidths(widths);
                            break;
                        default:
                            widths = new float[] { 60f, 60f, 80f, 60f, 70f };
                            table.SetWidths(widths);
                            break;
                    }
                    ...
                    foreach (var item in customerList)
                    {
                        ...
                        if (IsDefect == true)
                        {
                            if (item.GetType().GetProperty("AllPictures").GetValue(item).ToString() != null)
                            {
                                var st = item.GetType().GetProperty("AllPictures").GetValue(item).ToString();
                                var list1 = st.Replace(" ", "").Split(',').ToList();
                                foreach (var crpimages in list1)
                                {
                                    RootDirectory = RootDirectory + vehiclePath + crpimages;
                                    ...
                                    if (Directory.GetFiles(IsFileExist, crpimages).Length > 0)
                                    {
                                        //string cropimage = System.Web.HttpContext.Current.Server.MapPath("~/Images/QRCVehicle/" + crpimages);
                                        iTextSharp.text.Image jpg = iTextSharp.text.Image.GetInstance(RootDirectory);
                                        jpg.Alignment = 3;
                        ...
                        if (item.GetType().GetProperty("CroppedPicture").GetValue(item).ToString() != null
                                   && item.GetType().GetProperty("CroppedPicture").GetValue(item).ToString() != ""
                                    && item.GetType().GetProperty("CroppedPicture").GetValue(item).ToString() != "null")
                        {
                            var st = item.GetType().GetProperty("CroppedPicture").GetValue(item).ToString();
                            var list1 = st.Replace(" ", "").Split(',').ToList();
                            foreach (var crpimages in list1)
                            {
                                RootDirectory = RootDirectory + vehiclePath + crpimages;
                        ...
                        if (IsDefect == false)
                        {
                            if (item.GetType().GetProperty("CapturedPicture").GetValue(item).ToString() != null
                                    && item.GetType().GetProperty("CapturedPicture").GetValue(item).ToString() != ""
                                    && item.GetType().GetProperty("CapturedPicture").GetValue(item).ToString() != "null")
                            {
                                var st = item.GetType().GetProperty("CapturedPicture").GetValue(item).ToString();
                                var list1 = st.Replace(" ", "").Split(',').ToList();
                                foreach (var crpimages in list1)
                                {
                                    RootDirectory = RootDirectory + vehiclePath + crpimages;
                                    ...
                                    //string cropimage = System.Web.HttpContext.Current.Server.MapPath("~/Images/QRCVehicle/" + crpimages);
                                    //iTextSharp.text.Image jpg = iTextSharp.text.Image.GetInstance(cropimage);
                                    //jpg.Alignment = 3;
                                    //jpg.SpacingBefore = 5f;
                                    //jpg.ScaleToFit(100f, 100f);
                                    //jpg.SpacingAfter = 1f;
        ...
        [ActionName("GetFuelReport")]
        public ActionResult FuelFilled()
        {
            eTracLoginModel objLoginSession = new eTracLoginModel();
            if ((eTracLoginModel)Session["eTrac"] != null)
            {
            ...
            _fromDate = _fromDate.ConvertClientTZtoUTC();
            _toDate = _toDate.ConvertClientTZtoUTC();
            var objeFleetFuelingModelForServiceList = _IReportManager.GetFuelingReport(objLoginSession.UserId, objLoginSession.LocationID, _fromDate, _toDate, null, null, "");
            return PartialView("_Fueling", objeFleetFuelingModelForServiceList);

        }

        /// <summary>Created by Bhushan Dod on 29/12/17
        /// Get details of 
        /// </summary>
        /// <param name="LocationId,UserId,FromDate,ToDate"></param>
        /// <returns></returns>
        public JsonResult GetAllFuelingDetails(string fuelType, DateTime? fromDate, DateTime? toDate, long? userId)
        {
            ...
            _fromDate = _fromDate.ConvertClientTZtoUTC();
            _toDate = _toDate.ConvertClientTZtoUTC();
            var FuelJQGridList = _IReportManager.GetFuelingReport(objLoginSession.UserId, objLoginSession.LocationID, _fromDate, _toDate, Convert.ToInt64(fuelType), userId, "");

            return Json(FuelJQGridList, JsonRequestBehavior.AllowGet);
        ...
        [ActionName("GetDriverDetailsReport")]
        public ActionResult DriverDetails()
        {
            ...
            _fromDate = _fromDate.ConvertClientTZtoUTC();
            _toDate = _toDate.ConvertClientTZtoUTC();
            var objeFleetDriverDetails = _IReportManager.GetDriverDetailsReport(objLoginSession.UserId, objLoginSession.LocationID, _fromDate, _toDate, null, "");
            return PartialView("_DriverDetails", objeFleetDriverDetails);

        }

        /// <summary>Created by Bhushan Dod on 31/12/17
        /// Get details of 
        /// </summary>
        /// <param name="LocationId,UserId,FromDate,ToDate"></param>
        /// <returns></returns>
        public JsonResult GetAllDriverDetails(string search, DateTime? fromDate, DateTime? toDate, long? userId)
        {
            ...
            _fromDate = _fromDate.ConvertClientTZtoUTC();
            _toDate = _toDate.ConvertClientTZtoUTC();
            var objeFleetDriverDetails = _IReportManager.GetDriverDetailsReport(objLoginSession.UserId, objLoginSession.LocationID, _fromDate, _toDate, userId, "");

            return Json(objeFleetDriverDetails, JsonRequestBehavior.AllowGet);

        }
        #endregion Driver Details

        #region Maintenance 
        [ActionName("GetMaintenanceDetails")]
        public ActionResult MaintenanceDetails()
        {
            ...
            _fromDate = _fromDate.ConvertClientTZtoUTC();
            _toDate = _toDate.ConvertClientTZtoUTC();
            var objeFleetMainenanceDetails = _IReportManager.GeteMaintenanceDetailsReport(objLoginSession.UserId, objLoginSession.LocationID, _fromDate, _toDate, null, "");
            return PartialView("_MaintenanceDetails", objeFleetMainenanceDetails);
        }
        ...
        /// <param name="LocationId,UserId,FromDate,ToDate"></param>
        /// <returns></returns>
        public JsonResult GetAllMaintenanceDetails(string search, DateTime? fromDate, DateTime? toDate, long? userId)
        {
            ...
            _fromDate = _fromDate.ConvertClientTZtoUTC();
            _toDate = _toDate.ConvertClientTZtoUTC();
            var objeFleetMainenanceDetails = _IReportManager.GeteMaintenanceDetailsReport(objLoginSession.UserId, objLoginSession.LocationID, _fromDate, _toDate, userId, "");

            return Json(objeFleetMainenanceDetails, JsonRequestBehavior.AllowGet);

        }
        #endregion Maintenance

        #region Incident Report 
        [ActionName("GetIncidentDetails")]
        public ActionResult IncidentDetails()
        {
            ...
            _fromDate = _fromDate.ConvertClientTZtoUTC();
            _toDate = _toDate.ConvertClientTZtoUTC();
            var objeFleetMainenanceDetails = _IReportManager.GetIncidentDetailsReport(objLoginSession.UserId, objLoginSession.LocationID, _fromDate, _toDate, null, "");
            return PartialView("_IncidentDetails", objeFleetMainenanceDetails);
        }
        ...
        /// <param name="LocationId,UserId,FromDate,ToDate"></param>
        /// <returns></returns>
        public JsonResult GetAllIncidentDetails(string search, DateTime? fromDate, DateTime? toDate, long? userId)
        {
            ...
            _fromDate = _fromDate.ConvertClientTZtoUTC();
            _toDate = _toDate.ConvertClientTZtoUTC();
            var objeFleetMainenanceDetails = _IReportManager.GetIncidentDetailsReport(objLoginSession.UserId, objLoginSession.LocationID, _fromDate, _toDate, userId, "");

            return Json(objeFleetMainenanceDetails, JsonRequestBehavior.AllowGet);

        }
        #endregion Incident Report

        #region Vehicle Report 
        [ActionName("GetVehicleReport")]
        public ActionResult Vehicle()
        {
            ...
            _fromDate = _fromDate.ConvertClientTZtoUTC();
            _toDate = _toDate.ConvertClientTZtoUTC();
            var objeFleetFuelingModelForServiceList = _IReportManager.GetVehicleDetailsReport(objLoginSession.UserId, objLoginSession.LocationID, _fromDate, _toDate, null, null, "");
            return PartialView("_VehicleDetails", objeFleetFuelingModelForServiceList);
        }
        ...
        /// <param name="LocationId,UserId,FromDate,ToDate"></param>
        /// <returns></returns>
        public JsonResult GetAllVehicleDetails(string fuelType, DateTime? fromDate, DateTime? toDate, long? userId)
        {
            ...
            _fromDate = _fromDate.ConvertClientTZtoUTC();
            _toDate = _toDate.ConvertClientTZtoUTC();
            var FuelJQGridList = _IReportManager.GetVehicleDetailsReport(objLoginSession.UserId, objLoginSession.LocationID, _fromDate, _toDate, Convert.ToInt64(fuelType), userId, "");

            return Json(FuelJQGridList, JsonRequestBehavior.AllowGet);

        }
        #endregion Vehicle Report

        #region Mileage Report 
        [ActionName("GetMileageReport")]
        public ActionResult MileageReport()
        {
            ...
            _fromDate = _fromDate.ConvertClientTZtoUTC();
            _toDate = _toDate.ConvertClientTZtoUTC();
            var objeFleetFuelingModelForServiceList = _IReportManager.GetMileageReport(objLoginSession.UserId, objLoginSession.LocationID, _fromDate, _toDate, null, null, "");
            return PartialView("_MileageDetails", objeFleetFuelingModelForServiceList);
        ...
        /// <param name="LocationId,UserId,FromDate,ToDate"></param>
        /// <returns></returns>
        public JsonResult GetMileageDetails(string fuelType, DateTime? fromDate, DateTime? toDate, long? userId)
        {
            ...
            _fromDate = _fromDate.ConvertClientTZtoUTC();
            _toDate = _toDate.ConvertClientTZtoUTC();
            var MileageList = _IReportManager.GetMileageReport(objLoginSession.UserId, objLoginSession.LocationID, _fromDate, _toDate, Convert.ToInt64(fuelType), userId, "");

            return Json(MileageList, JsonRequestBehavior.AllowGet);

        }
        #endregion Mileage Report 

        #region Damage Report
        public ActionResult GeteFleetDamageDetails(string vehicle, DateTime? fromDate, DateTime? toDate, [Bind(Prefix = "eT")] string exportingType = null,
         [Bind(Prefix = "sC")] string sortCol = null, [Bind(Prefix = "sD")] string sortDir = null)
        {
            eTracLoginModel objLoginSession = new eTracLoginModel();
            if ((eTracLoginModel)Session["eTrac"] != null)
            {
            ...
            _fromDate = _fromDate.ConvertClientTZtoUTC();
            _toDate = _toDate.ConvertClientTZtoUTC();
            var lstdata = _IReportManager.GeteFleetDamageList(objLoginSession.LocationID, objLoginSession.UserId, _fromDate, _toDate, Convert.ToInt64(vehicle)).ToList();
            //shubham 07112016 code for PDF Export
            if (exportingType == "PDF")
            {
                DateTime FD = Convert.ToDateTime(fromDate);
                ...
                switch (sortCol)
                {
                    ...
                    case "Vehicle No":
                        if (sortDir == "asc")
                            lstdata = lstdata.OrderBy(o => o.VehicleNumber).ToList();
                        else
                            lstdata = lstdata.OrderByDescending(o => o.VehicleNumber).ToList();
                        break;
                    case "Description":
                        if (sortDir == "asc")
                            lstdata = lstdata.OrderBy(o => o.DamageDescription).ToList();
                        else
                            lstdata = lstdata.OrderByDescending(o => o.DamageDescription).ToList();
                        break;
                    case "Reported By":
                        if (sortDir == "asc")
                            lstdata = lstdata.OrderBy(o => o.DriverName).ToList();
                        else
                            lstdata = lstdata.OrderByDescending(o => o.DriverName).ToList();
                        break;
                    case "Date of Damage":
                        if (sortDir == "asc")
                            lstdata = lstdata.OrderBy(o => o.CreatedDate).ToList();
                ...
                string fileName = StringExtensionMethods.AppendTimeStamp(ReportName.eFleetDamage + ".pdf");
                string RootPath = Server.MapPath("~/");
                string RootDirectory = Server.MapPath("~/ReportPDF/FullPageReport/eFleet/");
                if (!Directory.Exists(RootDirectory))
                {
                    Directory.CreateDirectory(RootDirectory);
                }
                string filePath = RootDirectory + fileName;
                ExporteFleetPDFDetail<eFleetDamageReportModel>(lstdata, new string[] { "QRCodeID", "VehicleNumber", "DamageDescription", "DriverName", "CreatedDate" }, new string[] { "QR Code ID", "Vehicle No", "Description", "Reported By", "Date of Damage" },
            filePath, PDFReportNameHeading.Damage, true, false, fDate, tDate);

                promptBoxForPDFFileSave(filePath, fileName);
                return null;
            }
            else
                return PartialView("_eFleetDamage", lstdata);
            //end 07112016
        }

        public ActionResult DamageDownload(string Id)
        {
            try
            {
                if (!string.IsNullOrEmpty(Id))
                {
                    ...
                    var _eFleetFuelModel = _IReportManager.GeteFleetVehicleMasterLogDetailsById(Convert.ToInt64(Id));
                    if (_eFleetFuelModel != null)
                    {
                        ...
                        string filePath = RootDirectory + fileName;
                        var objList = new List<eFleetDamageReportModel>();
                        objList.Add(_eFleetFuelModel);
                        ExporteFleetPDFDetail<eFleetDamageReportModel>(objList, new string[] { "QRCodeID", "VehicleNumber", "DamageDescription", "DriverName", "CreatedDate" }, new string[] { "QR Code ID", "Vehicle No", "Description", "Reported By", "Date of Damage" },
                    filePath, PDFReportNameHeading.Damage, false, false, null, null);
        ...
        #endregion Damage Report

        #region Inspection Report
        [ActionName("GetInspectionReport")]
        public ActionResult GeteFleetInspectionDetails()
        {
            eTracLoginModel objLoginSession = new eTracLoginModel();
            if ((eTracLoginModel)Session["eTrac"] != null)
            {
            ...
            _fromDate = _fromDate.ConvertClientTZtoUTC();
            _toDate = _toDate.ConvertClientTZtoUTC();
            var lstdata = _IReportManager.GeteFleetInspectionList(objLoginSession.LocationID, objLoginSession.UserId, _fromDate, _toDate, null).ToList();
            return PartialView("_Inspection", lstdata);
        }

        /// <summary>Created by Bhushan Dod on 29/1/18
        /// Get details of 
        /// </summary>
        /// <param name="LocationId,UserId,FromDate,ToDate"></param>
        /// <returns></returns>
        public JsonResult GetInspectionDetails(string vehicle, DateTime? fromDate, DateTime? toDate)
        {
            ...
            _fromDate = _fromDate.ConvertClientTZtoUTC();
            _toDate = _toDate.ConvertClientTZtoUTC();
            var lstdata = _IReportManager.GeteFleetInspectionList(objLoginSession.LocationID, objLoginSession.UserId, _fromDate, _toDate, Convert.ToInt64(vehicle)).ToList();

            return Json(lstdata, JsonRequestBehavior.AllowGet);

        }
        #endregion Inspection Report

        #region Defects Report
        [ActionName("GetDefectReport")]
        public ActionResult GeteFleetDefectReport(string vehicle,DateTime? fromDate, DateTime? toDate)
        {
            eTracLoginModel objLoginSession = new eTracLoginModel();
            if ((eTracLoginModel)Session["eTrac"] != null)
            {
            ...
            _fromDate = _fromDate.ConvertClientTZtoUTC();
            _toDate = _toDate.ConvertClientTZtoUTC();
            var lstdata = _IReportManager.GeteFleetDefectList(objLoginSession.LocationID, objLoginSession.UserId, _fromDate, _toDate, null).ToList();
            return PartialView("_Defect", lstdata);
        }
        //public ActionResult GeteFleetDamageDetails(string vehicle, DateTime? fromDate, DateTime? toDate, [Bind(Prefix = "eT")] string exportingType = null,
        // [Bind(Prefix = "sC")] string sortCol = null, [Bind(Prefix = "sD")] string sortDir = null)
        //{
        ...
        //        string filePath = RootDirectory + fileName;
        //        ExporteFleetPDFDetail<eFleetDamageReportModel>(lstdata, new string[] { "QRCodeID", "VehicleNumber", "DamageDescription", "DriverName", "CreatedDate" }, new string[] { "QR Code ID", "Vehicle No", "Description", "Reported By", "Date of Damage" },
        //    filePath, PDFReportNameHeading.Damage, true, fDate, tDate);
        ...
        /// Created For : To Make a PDF of defect Images by Id
        /// </summary>
        /// <param name="Id"></param>
        /// <returns></returns>
        public ActionResult DefectDownload(long Id)
        {
            try
            {
                if (Id > 0)
                {
                    ...
                    var _eFleetDefectModel = _IReportManager.GeteFleetDefectDetailsById(Convert.ToInt64(Id));
                    if (_eFleetDefectModel != null)
                    {
                        string fileName = StringExtensionMethods.AppendTimeStamp(ReportName.eFleetDefect + ".pdf");
                        string RootPath = Server.MapPath("~/");
                        string RootDirectory = Server.MapPath("~/ReportPDF/FullPageReport/eFleet/");
                        if (!Directory.Exists(RootDirectory))
                        {
                        ...
                        string filePath = RootDirectory + fileName;
                        var objList = new List<eFleetDefectReportModel>();
                        objList.Add(_eFleetDefectModel);
                        ExporteFleetPDFDetail<eFleetDefectReportModel>(objList, new string[] { "QrcodeId", "VehicleNumber", "ProjectDescripton", "UserName", "InspectionType", "DefectType", "CreatedOn" }, new string[] { "QR Code ID", "Vehicle No", "Description", "Reported By","Inspection Type","Defect Type", "Date of Defect" },
                    filePath, PDFReportNameHeading.Defect, false, true, null, null);
        ...
        #endregion Defects Report

        #region Passenger Count Report
        [ActionName("PassengerCountReport")]
        public ActionResult PassengerCountReport()
        {
            return PartialView("_PassengerCount");
        }
        public JsonResult PassengerCountReportData(DateTime? fromDate=null, DateTime? toDate = null, string week=null,string Month=null,string Year=null)
        {
            eTracLoginModel objLoginSession = new eTracLoginModel();
            if ((eTracLoginModel)Session["eTrac"] != null)
            {
            ...
            _toDate = _toDate.ConvertClientTZtoUTC();
            //maintaining flag  if interval date come then need to fetch record till midnight of todate day
            if (fromDate == null && toDate == null && week == null && Month == null && Year == null)
            {
                currentUtcDate = clientdt.ConvertClientTZtoUTC();
                currentWeek = currentUtcDate.DayOfYear / 7;
                
            }
            var lstdata = _IReportManager.GeteFleetPassengerCountHoursOfWeekDays(objLoginSession.LocationID, objLoginSession.UserId, _fromDate, _toDate, currentWeek).ToList();
            return Json(lstdata, JsonRequestBehavior.AllowGet);
            //return PartialView("_PassengerCount", lstdata);
        }
        #endregion Passenger Count Report
        #endregion eFleet

        #region Idle Employee Report
        [ActionName("IdleEmployee")]
        public ActionResult IdleEmployeeReport()
        {
            return PartialView("_IdleEmployeeReport");
        }
        public JsonResult IdleEmployeeReport(long? locationId, long? userId, DateTime? fromDate, DateTime? toDate, string fromMonth, string toMonth)
        {
            eTracLoginModel objLoginSession = new eTracLoginModel();
            if ((eTracLoginModel)Session["eTrac"] != null)
            {
            ...
            //Getting client date time. 
            var fromMonthUtc = Convert.ToDateTime(fromMonth);
            var toMonthUtc = Convert.ToDateTime(toMonth);
            var clientdt = DateTime.UtcNow.GetClientDateTimeNow();
            bool isUTCDay = true;
            DateTime _fromDate = fromDate ?? clientdt.Date;
            DateTime _toDate = toDate ?? clientdt.AddDays(1).Date;
            ...
            _fromDate = _fromDate.ConvertClientTZtoUTC();
            _toDate = _toDate.ConvertClientTZtoUTC();
            var lstdata = _IReportManager.IdleEmployeeList(objLoginSession.LocationID, userId, _fromDate, _toDate, fromMonthUtc, toMonthUtc).ToList();
            return Json(lstdata, JsonRequestBehavior.AllowGet);
        }

        public JsonResult IdleEmployeeReportForWeek(long? locationId, long? userId, int weekNumber)
        {
            eTracLoginModel objLoginSession = new eTracLoginModel();
            if ((eTracLoginModel)Session["eTrac"] != null)
            {
            ...
            var lstdata = _IReportManager.IdleEmployeeListForWeek(objLoginSession.LocationID, userId, weekNumber).ToList();
            return Json(lstdata, JsonRequestBehavior.AllowGet);
        }


        [ActionName("IdleEmployeeReportTrendz")]
        public JsonResult IdleEmployeeReportTrendz(long? locationId, long? userId, DateTime? fromDate, DateTime? toDate)
        {
            eTracLoginModel objLoginSession = new eTracLoginModel();
            if ((eTracLoginModel)Session["eTrac"] != null)
            {
            ...
            _fromDate = _fromDate.ConvertClientTZtoUTC();
            _toDate = _toDate.ConvertClientTZtoUTC();
            var lstdata = _IReportManager.IdleEmployeeListForSelectedDateRange(objLoginSession.LocationID, userId, _fromDate, _toDate).ToList();
            return Json(lstdata);
        }

        public JsonResult GetEmployeeIdleDetails(long userId, string UserName, DateTime? fromDate, DateTime? toDate, [Bind(Prefix = "eT")] string exportingType = null,
            [Bind(Prefix = "sC")] string sortCol = null, [Bind(Prefix = "sD")] string sortDir = null, [Bind(Prefix = "h1")] string highChart1 = null,
            [Bind(Prefix = "h2")] string highChart2 = null, [Bind(Prefix = "iT")] bool isTable = false)
        {
            eTracLoginModel objLoginSession = new eTracLoginModel();
            ...
            //  long _userId = 0;
            List<ReportChartIdleEmployee> lstdata = new List<ReportChartIdleEmployee>();
            if (toDate != null)
            {
                if (toDate.Value.ToLongTimeString() == "12:00:00 AM")
                {
            ...
            //    }
            //}
            lstdata = _IReportManager.IdleEmployeeListDataByEmployee(userId, UserName, objLoginSession.LocationID, fromDate, toDate).ToList();
            //Below code is for if click on FullPage PDF Export
            if (exportingType == "PDF")
            {
                DateTime FD = Convert.ToDateTime(fromDate);
                ...
                string tDate = TD.ToString("MMMM dd, yyyy hh:mm tt");
                string fileName = StringExtensionMethods.AppendTimeStamp(ReportName.IdleEmployee + ".pdf");
                string RootPath = Server.MapPath("~/");
                string RootDirectory = Server.MapPath("~/ReportPDF/FullPageReport/IdlePDF/");
                if (!Directory.Exists(RootDirectory))
                {
                    Directory.CreateDirectory(RootDirectory);
                }
                ...
                if (isTable == true)
                {
                    switch (sortCol)
                    {
                        case "User Name":
                            if (sortDir == "asc")
                                lstdata = lstdata.OrderBy(o => o.UserName).ToList();
                            else
                                lstdata = lstdata.OrderByDescending(o => o.UserName).ToList();
                            break;
                        case "Total":
                            if (sortDir == "asc")
                                lstdata = lstdata.OrderBy(o => o.IdleTotalTime).ToList();
                            else
                                lstdata = lstdata.OrderByDescending(o => o.IdleTotalTime).ToList();
                            break;
                        case "Start Time":
                            if (sortDir == "asc")
                                lstdata = lstdata.OrderBy(o => o.StartTime).ToList();
                            else
                                lstdata = lstdata.OrderByDescending(o => o.StartTime).ToList();
                            break;
                        case "End Time":
                            if (sortDir == "asc")
                                lstdata = lstdata.OrderBy(o => o.EndTime).ToList();
                            else
                                lstdata = lstdata.OrderByDescending(o => o.EndTime).ToList();
                            break;
                        case "Time":
                            if (sortDir == "asc")
                                lstdata = lstdata.OrderBy(o => o.DifferenceTime).ToList();
                            else
                                lstdata = lstdata.OrderByDescending(o => o.DifferenceTime).ToList();
                            break;
                        case "Location":
                            if (sortDir == "asc")
                                lstdata = lstdata.OrderBy(o => o.LocationName).ToList();
                            else
                                lstdata = lstdata.OrderByDescending(o => o.LocationName).ToList();
                            break;
                        default:
                            lstdata = lstdata.OrderByDescending(o => o.IdleTotalTime).ToList();
                            break;
                    }
                    ExportPDF<ReportChart>(lstdata as IList<ReportChart>, new string[] { "UserName", "IdleTotalTime", "StartTime", "EndTime", "DifferenceTime", "LocationName" },
                        new string[] { "User Name", "Total", "Start Time", "End Time", "Time", "Location" },
                        filePath, PDFReportNameHeading.IdleEmployee, fDate, tDate, highChart1, highChart2, isTable, "Idle Emloyee " + UserName);
                }
                else
                {
                    ExportPDF<ReportChart>(lstdata as IList<ReportChart>, new string[] { "UserName", "IdleTotalTime", "StartTime", "EndTime", "DifferenceTime", "LocationName" },
        ...
        #endregion Idle Employee Report       
    }
}
class myClass
{
