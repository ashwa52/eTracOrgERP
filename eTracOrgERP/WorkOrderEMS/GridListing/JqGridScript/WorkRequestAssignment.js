...
var workrequestProjectTypeWO = ''
    + '&nbsp&nbsp<select id="workRequestProjectTypedll" class="" onchange="doSearch(arguments[0]||event);">'
    + '<option value="">Select All</option>'
    + '<option value="128">Work Request</option>'
    + '<option value="129">Special Project</option>'
    + '<option value="256">Facility Request</option>'
    + '<option value="279">	Continuous Request</option> </select>';
...
+ '<option value="102">Non Asset Type</option></select>'; //this will need to uncomment once app is live
...
$(function () {
    ...
    $("#tbl_WorkRequestAssignmentList").jqGrid({
        ...
        colNames: ['Work Order ID', 'WorkRequestType', 'WorkRequest Type', 'Asset', 'LocationID', 'LocationName', 'Problem Desc', 'PriorityLevel', 'Priority Level', 'WorkRequestImage', 'SafetyHazard', 'Project Desc', 'WorkRequestStatus', 'Status', 'QRC Type', 'RequestBy', 'AssignToUserId', 'Assign To User', 'AssignByUserId', 'Remarks', 'WorkRequestProjectType', 'Project Type', 'Facility Request Type', 'Profile Image', 'Work Order Image', 'Submitted On', 'Submitted By', 'Submitted By User', 'DisclaimerForm', 'SurveyForm', 'StartDate', 'EndDate', 'WeekDays', 'StartTime', 'AssignedTime', 'CustomerName', 'VehicleMake', 'VehicleModel', 'CustomerContact', 'VehicleYear', 'VehicleColor', 'DriverLicenseNo', 'TotalTime', 'ConStartTime', 'Actions'],
        colModel: [{ name: 'CodeID', width: 100, sortable: true, hidden: false },
                   { name: 'WorkRequestType', width: 0, sortable: true, hidden: true },
                   { name: 'WorkRequestTypeName', width: 0, sortable: false, hidden: true },
                   { name: 'AssetID', width: 0, sortable: false, hidden: true },
                   { name: 'LocationID', width: 0, sortable: true, hidden: true },
                   { name: 'LocationName', width: 0, sortable: false, hidden: true },
                   { name: 'ProblemDesc', width: 0, sortable: false, hidden: true },
                   { name: 'PriorityLevel', width: 0, sortable: true, hidden: true },
                   { name: 'PriorityLevelName', width: 100, sortable: true, hidden: false },
                   { name: 'WorkRequestImage', width: 0, sortable: false, hidden: true },
                   { name: 'SafetyHazard', width: 0, sortable: false, hidden: true },
                   { name: 'ProjectDesc', width: 0, sortable: false, hidden: true },
                   { name: 'WorkRequestStatus', width: 0, sortable: true, hidden: true },
                   { name: 'WorkRequestStatusName', width: 100, sortable: false, hidden: false },
                   { name: 'QRCType', width: 100, sortable: false, hidden: false },
                   { name: 'RequestBy', width: 0, sortable: false, hidden: true },
                   { name: 'AssignToUserId', width: 0, sortable: false, hidden: true },
                   { name: 'AssignToUserName', width: 100, sortable: false, hidden: false },
                   { name: 'AssignByUserId', width: 0, sortable: false, hidden: true },
                   { name: 'Remarks', width: 0, sortable: false, hidden: true },
                   { name: 'WorkRequestProjectType', width: 0, sortable: false, hidden: true },
                   { name: 'WorkRequestProjectTypeName', width: 100, sortable: false },
                   { name: 'FacilityRequestType', width: 100, sortable: false },
                   { name: 'ProfileImage', width: 100, sortable: false, formatter: imageFormat },
                   { name: 'AssignedWorkImage', width: 100, sortable: false, formatter: imageFormat },
                   { name: 'CreatedDate', width: 100, sortable: false },
                   { name: 'CreatedByProfile', width: 100, sortable: false, formatter: imageFormat },
                   { name: 'CreatedByUserName', width: 100, sortable: false },
                   { name: 'DisclaimerForm', width: 0, sortable: true, hidden: true },
                   { name: 'SurveyForm', width: 0, sortable: true, hidden: true },
                   { name: 'StartDate', width: 0, sortable: true, hidden: true },
                   { name: 'EndDate', width: 0, sortable: true, hidden: true },
                   { name: 'WeekDays', width: 0, sortable: true, hidden: true },
                   { name: 'StartTime', width: 0, sortable: true, hidden: true },
                   { name: 'AssignedTime', width: 0, sortable: true, hidden: true },
                   { name: 'CustomerName', width: 0, sortable: true, hidden: true },
                   { name: 'VehicleMake', width: 0, sortable: true, hidden: true },
                   { name: 'VehicleModel', width: 0, sortable: true, hidden: true },
                   { name: 'CustomerContact', width: 0, sortable: true, hidden: true },
                   { name: 'VehicleYear', width: 0, sortable: true, hidden: true },
                   { name: 'VehicleColor', width: 0, sortable: true, hidden: true },
                   { name: 'DriverLicenseNo', width: 0, sortable: true, hidden: true },
                   { name: 'TotalTime', width: 0, sortable: true, hidden: true },
                   { name: 'ConStartTime', width: 0, sortable: true, hidden: true },
                   { name: 'act', index: 'act', width: 120, sortable: false, hidden: (($_isEdit != undefined && (parseInt($_isEdit) == 1)) ? false : true) }],
        rownum: 10,
        rowList: [10, 20, 30],
        scrollOffset: 0,
        ...
        caption: '<div class="header_search"><input id="txtSearch" class="inputSearch" placeholder="Search by Assign To User" onkeydown="doSearch(arguments[0]||event)" type="text">&nbsp;' + qRCTypeddl + '&nbsp;' + workrequestStatus + '' + workrequestProjectTypeWO + '' + allLocation + '</div>'
    });
...
//function doSearchForWO()
//{
//    debugger
//    var woProjectType = jQuery("#workRequestProjectTypedll option:selected").val();
...
//    }
//    jQuery("#tbl_WorkRequestAssignmentList").jqGrid('setGridParam', { url: '../GridListing/JqGridHandler/WorkRequestAssignmentList.ashx?UserID=' + $_UserID + '&txtSearch=' + txtSearch + "&locationId=" + locaId + '&filter=' + workStauts + '&filterqrc=' + QRCType, page: 1 }).trigger("reloadGrid");

//}
function gridReload() {
    var txtSearch = jQuery("#txtSearch").val();
    var workStauts = jQuery("#workRequestStatusTypedll option:selected").text();
    var QRCType = jQuery("#SearchQRCType option:selected").val();
    var woProjectType = jQuery("#workRequestProjectTypedll option:selected").val();
    var locaId = $('#ViewAllLocation').prop('checked') == true ? 0 : $("#drp_MasterLocation :selected").val();
    ...
    jQuery("#tbl_WorkRequestAssignmentList").jqGrid('setGridParam', { url: '../GridListing/JqGridHandler/WorkRequestAssignmentList.ashx?UserID=' + $_UserID + '&txtSearch=' + txtSearch + "&locationId=" + locaId + '&filter=' + workStauts + '&filterqrc=' + QRCType + '&filterwrtype=' + woProjectType, page: 1 }).trigger("reloadGrid");
...
$("#tbl_WorkRequestAssignmentList tr td:nth-child(25)").width(140);
$("#workorderDetail").live("click", function (event) {
    ...
    if (WorkRequestProjectType == 128 && QRCType != 'N/A' && QRCType != '') {
        $("#lblQRCType").html(QRCType);
        $("#labelQRCType").show();
        $("#lblQRCType").show();
    } else {
        $("#labelQRCType").hide();
