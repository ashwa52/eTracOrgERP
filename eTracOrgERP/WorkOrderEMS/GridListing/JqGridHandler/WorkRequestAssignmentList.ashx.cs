        ...
        private string BuildJQGridResults(List<WorkOrderEMS.Models.CommonModels.WorkRequestAssignmentModelList> WorkRequestList, int numberOfRows, int pageIndex, int TotalRecords)
        {
            ...
            try
            {
                foreach (var WorkRequest in WorkRequestList)
                {
                    ...
                    row.cell[14] = String.IsNullOrEmpty(WorkRequest.QRCType) ? ((WorkRequest.eFleetVehicleID != null && WorkRequest.eFleetVehicleID != ""?"Shuttle Bus":"N/A")) : WorkRequest.QRCType + " (" + WorkRequest.QRCodeID + ")";
                    row.cell[15] = Convert.ToString(WorkRequest.RequestBy);
                    row.cell[16] = Convert.ToString(WorkRequest.AssignToUserId);
                    row.cell[17] = WorkRequest.AssignToUserName;
                    row.cell[18] = Convert.ToString(WorkRequest.AssignByUserId);
                    row.cell[19] = WorkRequest.Remarks;
                    row.cell[20] = Convert.ToString(WorkRequest.WorkRequestProjectType);
                    row.cell[21] = WorkRequest.WorkRequestProjectTypeName;
                    //row.cell[21] = WorkRequest.FacilityRequestType == null ? "N/A" : WorkRequest.FacilityRequestType;
                    row.cell[22] = (WorkRequest.FacilityRequestType == null || WorkRequest.FacilityRequestType.TrimWhiteSpace() == "" || WorkRequest.FacilityRequestType.Trim() == "") ? "N/A" : WorkRequest.FacilityRequestType;
                    row.cell[23] = WorkRequest.ProfileImage == null ? HostingPrefix + ConstantImages.Replace("~", "") + "no-profile-pic.jpg" : HostingPrefix + ProfilePicPath.Replace("~", "") + WorkRequest.ProfileImage;
                    row.cell[24] = WorkRequest.AssignedWorkOrderImage == null ? HostingPrefix + ConstantImages.Replace("~", "") + "no-asset-pic.png" : HostingPrefix + WorkRequestImagepath.Replace("~", "") + WorkRequest.AssignedWorkOrderImage;
                    row.cell[25] = WorkRequest.CreationDate; //CreatedDate.ToString("MM/dd/yyyy HH:mm:ss tt", CultureInfo.InvariantCulture);
                    row.cell[26] = WorkRequest.CreatedByProfile == null ? HostingPrefix + ConstantImages.Replace("~", "") + "no-profile-pic.jpg" : HostingPrefix + ProfilePicPath.Replace("~", "") + WorkRequest.CreatedByProfile;
                    row.cell[27] = WorkRequest.CreatedByUserName;
                    row.cell[28] = WorkRequest.DisclaimerForm;
                    row.cell[29] = WorkRequest.SurveyForm;
                    row.cell[30] = WorkRequest.StartDate;
                    row.cell[31] = WorkRequest.EndDate;
                    row.cell[32] = WorkRequest.WeekDays;
                    row.cell[33] = WorkRequest.StartTime;
                    row.cell[34] = WorkRequest.AssignedTime;
                    row.cell[35] = WorkRequest.CustomerName;
                    row.cell[36] = WorkRequest.VehicleMake;
                    row.cell[37] = WorkRequest.VehicleModel;
                    row.cell[38] = WorkRequest.CustomerContact;
                    row.cell[39] = WorkRequest.VehicleYear.ToString();
                    row.cell[40] = WorkRequest.VehicleColor;
                    row.cell[41] = WorkRequest.DriverLicenseNo;
                    row.cell[42] = WorkRequest.TotalTime;
                    row.cell[43] = WorkRequest.ConStartTime;
                    rows.Add(row);
                }
                result.rows = rows.ToArray();
