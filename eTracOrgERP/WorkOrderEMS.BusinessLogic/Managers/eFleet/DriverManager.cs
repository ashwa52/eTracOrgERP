    ...
    public class DriverManager : IDriverEfleet
    {
        ...
        public eDriverDetails GetListDriverDetails(long? UserId, int? pageIndex, int? numberOfRows, string sortColumnName, string sortOrderBy, long? locationId, string textSearch, string statusType)
        {
            try
            {
                ...
                var Results = db.eFleetDrivers.Join(db.UserRegistrations, ed => ed.EmployeeName, ur => ur.UserId, (ed, ur) => new { ed, ur }).
                    Where(a => a.ed.IsDeleted == false
                        && (((textSearch == null ? null : textSearch) == null || a.ed.DriverLicenseNo.Contains(textSearch))
                                  ...
                                  || ((textSearch == null ? null : textSearch) == null || (a.ur.FirstName + " " + a.ur.LastName).Contains(textSearch))
                                )
                ).Select(a => new eFleetDriverModel()
                {
                    DriverID = a.ed.DriverID,
                    DriverLicenseNo = a.ed.DriverLicenseNo,
                    CDLType = a.ed.CDLType,
                    EmployeeNameList = a.ur.FirstName + " " + a.ur.LastName,
                    CDLExpiration = a.ed.CDLExpiration,
                    MedicleCardExpiration = a.ed.MedicleCardExpiration,
                    MVRExpiration = a.ed.MVRExpiration,
                    DriverImage = (a.ur.ProfileImage == null) ? "" : HostingPrefix + ProfilePicPath.Replace("~", "") + a.ur.ProfileImage
