    ...
    public class eFleetCommonManager : IeFleetFuelingManager
    {
        ...
        public VehicleScanModel GeteFleetVehicleById(string QRCodeID, long locationId)
        {
            try
            {
                ...
                if (!(ObjVehicleDetail.VehicleID > 0))
        ...
        /// <returns></returns>
        public JQGridModel<FuelJQGridList> GetListeFleetFuelingForJQGridDetails(long? locationid, int? pageIndex, int? numberOfRows, string sortColumnName, string sortOrderBy, string textSearch, long? statusType, DateTime FromDate, DateTime ToDate)
        {
            try
            {
                workorderEMSEntities db = new workorderEMSEntities();
                ...
                var Results = db.eFleetFuelings.Where(a => (a.IsDeleted == false)
                            && (((locationid == 0) ? null : locationid) == null || a.LocationID == locationid)
                             ...
                             && (((textSearch == null ? null : textSearch) == null || a.VehicleNumber.Contains(textSearch))
                                  || ((textSearch == null ? null : textSearch) == null || a.DriverName.Contains(textSearch))
                                )
