    ...
    public class WorkRequestManager : IWorkRequestAssignment
    {
        ...
        public ServiceResponseModel<string> UpdateTaskStatus(ServiceWorkStatusModel obj)
        {
            ...
            try
            {
                    ...
                    )
                {
                    ...
                    var getData = ObjWorkRequestAssignmentRepository.GetSingleOrDefault(x => x.WorkRequestAssignmentID == obj.WorkRequestAssignmentID
                                                                          && x.IsDeleted == false);
                    if (getData != null && getData.WorkRequestProjectType == 279)
                    {
                        var currentDate = DateTime.UtcNow.Date;
                        var endDate = getData.EndDate.Value.Date;
                     
                        if (currentDate != endDate)
                        {
                            obj.EndDate = getData.EndDate.ToString();                                              
                        }
                        else
                        {
                            obj.EndDate = null;
                ...
                if (obj.LocationID != null)
                {
                    ...
                    if (authuser != null && authuser.UserId > 0)
                    {
                        ...
                        if (result != null || result > 0)
                        {
                            ...
                            ///This condition is used to send notification to all manager of sme locaion
                            ///For Normal Work Orders
                            if(obj.PriorityLevel == Convert.ToInt64(WorkRequestPriority.Level2Intermediate)
                                && obj.AssignToUserId == null)
                            {
                                var mgmList = objGlobalAdminManager.GetAllManagerList(obj.LocationID, authuser.UserId);
                                objEmailHelper.MailType = "WORKORDERREQUESTTOEMPLOYEE";
                                objEmailHelper.WorkRequestAssignmentID = obj.WorkRequestAssignmentID;
                                objEmailHelper.WorkOrderCodeId = obj.WorkrequestCode + result;
                                objEmailHelper.AssignedTime = obj.AssignedTime;
                                objEmailHelper.ProjectDesc = obj.ProblemDescription;
                                objEmailHelper.UserName = authuser.FirstName + " " + authuser.LastName;
                                
                                foreach (var ab in mgmList)
                                {
                                    if (ab.DeviceId != null)
                                    {
                                        objEmailHelper.IsWorkable = false;
                                        objEmailHelper.LocationName = ab.LocationName;
                                        PushNotificationFCM.FCMAndroid("New work order " + objEmailHelper.WorkOrderCodeId + " issued at " + objEmailHelper.LocationName, ab.DeviceId, objEmailHelper);
                                    }
                                }

                            }
                            ///THis condtion is used to send mail and notification to employee as well as manager 
                            ///of same location.
                            if (obj.PriorityLevel == 11)
                            {
                              List<EmailToManagerModel> objEmailReturn = new List<EmailToManagerModel>();
                                var objLocCode = objGlobalAdminManager.GetLocationById(Convert.ToInt64(obj.LocationID));
                                long userId = authuser.UserId;
                                var mgmList = objGlobalAdminManager.GetAllManagerList(obj.LocationID, authuser.UserId);
                                List<listForEmployeeDevice> res = objGlobalAdminManager.sendNotificaitonToAllEmployee(obj.LocationID, userId);
                                objEmailReturn = (from pd in _workorderEMSEntities.PermissionDetails
                                                  ...
                                                  join lm in _workorderEMSEntities.LocationMasters on obj.LocationID equals lm.LocationId
                                                  join urUserId in _workorderEMSEntities.UserRegistrations on userId equals urUserId.UserId
                                                  where mlm.LocationId == obj.LocationID && ur.IsDeleted == false
                                                         && ur.IsDeleted == false
                                                         && (pd.PermissionId == 3 || pd.PermissionId == 191) //eScanner
                                                          && pd.LocationId == obj.LocationID
                                ...
                                //objEmailReturn = objEmailLogRepository.SendEmailToManagerForUrgentWorkRequest(obj.LocationID, userId);//function return data according to location and username                               

                                if (objEmailReturn.Count() > 0)
                                {
                                    foreach (var item in objEmailReturn)
                                    {
                                        ////This function will use for send notification to All Employee who has assigned to selected location and has the permission of eMaintenance.                                                                                                                                       //List<listForEmployeeDevice> res = _IGlobalAdmin.sendNotificaitonToAllEmployee(obj.LocationID, obj.AssignByUserId);
                                        ...
                                        objEmailHelper.WorkRequestAssignmentID = result;
                                        objEmailHelper.WorkOrderCodeId = obj.WorkrequestCode + result;
                                        objEmailHelper.UserName = authuser.FirstName + " " + authuser.LastName;
                                        objEmailHelper.ProblemDesc = obj.ProblemDescription;
                                        objEmailHelper.LocationName = objLocCode.LocationName;
                                        objEmailHelper.LocationCode = objLocCode.Address2;
                                        //objEmailHelper.SafetyHazard = obj.SafetyHazard;
                                        ...
                                        if (IsSent == true)
                                        {
                                            ...
                                            try
                                            {
                                               ...
                                               // objEmailLogRepository.SaveEmailLog(objEmailog);
                                                _workorderEMSEntities.EmailLogs.Add(objEmailog);
                                    ...
                                    ///Commented By : Ashwajit Bansod As Getting exception when objEmailHelper set to be null
                                    //objEmailHelper = null;
                                    objEmailHelper.MailType = "URGENTWORKORDERREQUESTTOEMPLOYEE";
                                    objEmailHelper.WorkRequestAssignmentID = result;
                                    objEmailHelper.WorkOrderCodeId = obj.WorkrequestCode + result;
                                    ...
                                    objEmailHelper.LocationName = objLocCode.LocationName;
                                    objEmailHelper.LocationCode = objLocCode.Address2;
                                    //objEmailHelper.WorkOrderImage = obj.WorkRequestImage == null ? HostingPrefix + "/Content/Images/ProjectLogo/no-profile-pic.jpg" : HostingPrefix + (ConfigurationManager.AppSettings["WorkRequestImage"]).Replace("~", "") + obj.WorkRequestImage;
        ...
        public bool GetFacilityRequestByID(long WorkRequestAssignmentID, long LocationId, long UserId)
        {
            ...
            try
            {
                ...
                var t = ObjWorkRequestAssignmentRepository.GetAll(v => v.WorkRequestAssignmentID == WorkRequestAssignmentID && v.IsDeleted == false)
                                                          .FirstOrDefault();
                if (t.AssignToUserId == null || t.AssignToUserId == 0)
                {
                ...
                try
                {
                   ...
                   var time = DateTime.UtcNow;//Convert.ToDateTime("2015-06-07 09:35:00.000"); //DateTime.Now;
                    ...
                    foreach (var d in workDetails)
                    {
                        ...
                        objEmailHelper.WorkOrderImage = d.u.WorkRequestImage == null ? HostingPrefix + "/Content/Images/ProjectLogo/no-profile-pic.jpg" : HostingPrefix + (ConfigurationManager.AppSettings["WorkRequestImage"]).Replace("~", "") + d.u.WorkRequestImage;
                        ...
                        foreach (var date in dateList)
                        {
                            if (d.u.ConStartTime != null)
                            {
                                ...
                                {
                                    if (obj != null && d.u.WorkRequestAssignmentID > 0)
                                    {
                                        objData = repository.GetSingleOrDefault(x => x.WorkOrderID == d.u.WorkRequestAssignmentID && x.IsDeleted == false);
                                    }
                                    if (objData != null)
                                    {
                                        objData.IsRead = false;
                                    ...
                                    obj.WorkRequestStatus = Convert.ToInt64(WorkRequestStatus1.Missed);
                                    obj.WorkOrderCodeID = d.u.WorkOrderCodeID;
        ...
        public bool WorkOrderPauseResume(ServiceWorkOrderAcceptanceModel obj)
        {
            ...
            try
            {
                ObjWorkRequestAssignmentRepository = new WorkRequestAssignmentRepository();
                ObjUserRepository = new UserRepository();
                //var objlocation = new LocationMasterRepository();
                string Status = string.Empty;

                var authuser = ObjUserRepository.GetSingleOrDefault(x => x.ServiceAuthKey == obj.ServiceAuthKey && x.IsDeleted == false);
                //var locationDetails = objlocation.GetSingleOrDefault(x => x.LocationId == obj.LocationID && x.IsDeleted == false);
                if (authuser != null && authuser.UserId > 0 )
                {
                    ...
                    {    //If first time not pause
                        ...
                        //objDAR.ActivityDetails = DarMessage.DarUpdatePauseResumeTaskStatus(workDetails.WorkOrderCode+ workDetails.WorkOrderCodeID, locationDetails.LocationName, Status);
                        //objDAR.LocationId = obj.LocationID;
                        //objDAR.UserId = obj.UserId;
                        //objDAR.CreatedBy = obj.UserId;
                        //objDAR.CreatedOn = DateTime.UtcNow;
                        //objDAR.TaskType = obj.Status;
                        //Result result = ObjCommonMethodManager.SaveDAR(objDAR);
                        //#endregion Save DAR
            ...
            try
            {
                if (obj.LocationId > 0 && obj.ServiceAuthKey != null)
                {
                    ...
                    if (authuser != null && authuser.UserId > 0)
                    {
                        ...
                        if (workDetails != null && workDetails.AssignToUserId == 0 || workDetails.AssignToUserId == null)
                        {
                            workDetails.AssignToUserId = obj.UserId;
                            workDetails.AssignByUserId = workDetails.AssignByUserId;
                            workDetails.ModifiedBy = obj.UserId;
                            workDetails.ModifiedDate = DateTime.UtcNow;
                            ObjWorkRequestAssignmentRepository.SaveChanges();
